...
def table_rect(page,cord_from,cord_to):
...
def parse_table1(page,rect1):

    text=page.get_text(clip=rect1,sort=True)
...
def parse_table3(page,rect3):
    text=page.get_text(clip=rect3,sort=True)
    lines=[line.strip() for line in text.splitlines() if line.strip()]
...
def parse_table4(page,rect4):
    
    text=page.get_text(clip=rect4,sort=True)
